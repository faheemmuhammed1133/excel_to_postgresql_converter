...
export const uploadController = {
    ...
    processExcelFile: async (req, res) => {
        ...
        try {
            ...
            const tableName =
                req.body.tableName?.toLowerCase().replace(/\s+/g, "_") ||
                "excel_data";
            ...
            const sampleRow = rawData.find(
                (row) =>
                    row &&
                    Object.values(row).filter((v) => v !== null && v !== "")
                        .length > 0
            );

            if (!sampleRow) {
                throw new Error(
                    "No valid rows with data found in the Excel file"
                );
            }
            ...
            const columnDefs = sanitizedColumns.map((col, idx) => {
                ...
                if (typeof val === "number")
                    return `${col} ${
                        Number.isInteger(val) ? "INTEGER" : "NUMERIC"
                    }`;
